...
def year2label_fn(
    year: int, min_year: int, max_year: int, year_bucket_size: int = 2
) -> int:
    """
...
def label2year_fn(
    label: int, min_year: int, max_year: int, year_bucket_size: int = 2
) -> int:
    """Convert label to year."""
    year_range = max_year - min_year + 1
    num_buckets = year_range // year_bucket_size

    return int((label / num_buckets) * year_range + min_year)


def bodytype2label_fn(
    bodytype: str,
    possible_bodytypes: list = [
        "Convertible",
        "Coupe",
        "Hatchback",
        "MPV",
    ...
    def __getitem__(self, idx):
        ...
        if self.all_cars:
            image_file_path = (
                self.img_root_dir
                + "all_cars/"
                + row_of_interest["Brand_Name"]
                + "/"
                + str(row_of_interest["Model_Name"])
                + "/"
                + str(row_of_interest["Launch_Year"])
                + "/"
                + str(row_of_interest["Color"])
                + "/"
                + row_of_interest["file_path"]
            )
        else:
            image_file_path = (
                self.img_root_dir
        ...
        return (
            ...
            model_id,
            launch_year,
            self.bodytype2label_fn(bodytype),
            self.year2label_fn(year=launch_year),
            viewpoint,
        )
...
def inverse_transform(
    y: torch.Tensor, mean: list = img_rgb_mean, std: list = img_rgb_std
):
